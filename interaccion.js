...
/* ✏️ BOLSA DE BEATS (automático)
   Sube tus MP3 a la carpeta /audio del repositorio y el reproductor se actualiza SOLO
   (lee la carpeta con la API de GitHub). El título de cada beat = nombre del archivo .mp3.
   · BEATS_COVER : carátula común para TODOS los beats (una sola imagen en /images/beats)
   · BEATS_REPO  : usuario/repositorio de GitHub Pages donde vive la carpeta /audio */
const BEATS_COVER = "images/beats/portada.jpg";
const BEATS_REPO  = { user: "quasarmusik", repo: "quasarmusik.github.io", folder: "audio" };
/* ═══════════════════════════════════════════ */
...
window.onSpotifyIframeApiReady = (IFrameAPI) => {
  ...
  /* permite que el reproductor de beats pause Spotify */
  window.QuasarSpotify = { pauseAll(){ if(active>=0 && players[active] && players[active].ctrl){ try{players[active].ctrl.pause();}catch(e){} } } };
      ...
      controller.addListener("playback_update", (e) => {
        ...
        if (isPlaying) {
          ...
          if (active !== i) {
            ...
            active = i;
            if (window.QuasarBeats) window.QuasarBeats.pause();      /* pausa el reproductor de beats */
...
/* ════════════ REPRODUCTOR DE BEATS (HTML5 · carpeta /audio automática) ════════════ */
(function(){
  ...
  /* disponible para coordinar con Spotify (pausar el beat) */
  window.QuasarBeats={ pause(){ try{audio.pause();}catch(e){} } };
  ...
  function toggle(){ if(cur<0){ if(TRACKS.length) load(0,true); return; } audio.paused?audio.play().catch(()=>{}):audio.pause(); }

  btnPlay.addEventListener("click",toggle);
  btnPrev.addEventListener("click",()=>{ if(TRACKS.length) load(cur<=0?TRACKS.length-1:cur-1,true); });
  btnNext.addEventListener("click",()=>{ if(TRACKS.length) load(cur>=TRACKS.length-1?0:cur+1,true); });

  audio.addEventListener("play",()=>{ root.classList.add("playing"); if(window.QuasarSpotify) window.QuasarSpotify.pauseAll(); });
  audio.addEventListener("pause",()=>root.classList.remove("playing"));
  audio.addEventListener("ended",()=>{ if(TRACKS.length) load(cur>=TRACKS.length-1?0:cur+1,true); });
  ...
  function setTracks(tracks){
    TRACKS=tracks||[];
    if(!TRACKS.length){ elTitle.textContent="Próximamente"; elInfo.textContent=""; elCover.src=BEATS_COVER; list.innerHTML=""; cur=-1; return; }
    renderList();
    load(0,false);
  }

  /* Lee la carpeta /audio del repo (API de GitHub) y arma la lista en orden alfabético.
     Al subir un MP3 nuevo a /audio, aparece aquí automáticamente. */
  const api="https://api.github.com/repos/"+BEATS_REPO.user+"/"+BEATS_REPO.repo+"/contents/"+BEATS_REPO.folder;
  fetch(api,{headers:{Accept:"application/vnd.github+json"}})
    ...
    .then(files=>{
      const tracks=(Array.isArray(files)?files:[])
        ...
        .map(f=>({ title:f.name.replace(/\.mp3$/i,"").replace(/_/g," | "), info:"", cover:BEATS_COVER, src:BEATS_REPO.folder+"/"+encodeURIComponent(f.name) }));
      setTracks(tracks);
    })
    .catch(()=>setTracks([]));
})();
